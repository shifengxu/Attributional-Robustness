...
class AttackSaliency(nn.Module):
    def __init__(self,  config, normalize, n_classes=10, device=None):
    ...
    def topk_alignment_saliency_attack(self, model, images, y, min_value, max_value):
        ...
        adv = images.clone()
        with torch.no_grad():
            adv = adv + 2 * self.eps * (torch.rand(images.size()).cuda(device=self.device) - 0.5)
        ...
        g1_adp = torch.autograd.grad(top_scores.mean(), adv, retain_graph=True)[0]
        ...
        for i in range(self.num_steps):
            ...
            _, logits_with_softplus = model.forward({0:self.normalize_fn(adv) , 1: 1})
            ...
            btm_scores = logits_with_softplus.gather(1, index=torch.tensor(non_target_indices).cuda(device=self.device))
            btm_scores = btm_scores.max(dim = 1)[0]  # bottom scores
                
            g1 = torch.autograd.grad(top_scores.mean(), adv, retain_graph=True,create_graph=True)[0]
            g2 = torch.autograd.grad(btm_scores.mean(), adv, retain_graph=True,create_graph=True)[0]
    ...
    def topk_alignment_mean_saliency_attack(self, model, images, y, min_value, max_value):
        ...
        for i in range(self.num_steps):
            adv.requires_grad = True
            # 1:0 denotes relu and 1:1 denotes softplus
            logits_with_softplus, _ = model.forward({0: self.normalize_fn(adv), 1: 1})
            top_scores = logits_with_softplus.gather(1, index = y.unsqueeze(1))
            tmp_arr = np.array([[k for k in range(self.n_class) if k != y[j]] for j in range(y.size(0))])
            non_target_indices = torch.from_numpy(tmp_arr).cuda(device=self.device)
#             print(non_target_indices)
            btm_scores = logits_with_softplus.gather(1, index=non_target_indices)
    ...
    def saliency_attackIG(self, model , images, y , min_value , max_value  ):
        ...
        def counterfactual_gen(images):
            ...
            return counterfactuals.cuda(device=self.device)
...
#             print(measure)
            ...
            topK_loss = (saliency_flatten*elements1.cuda(device=self.device)).sum(1).mean()
            ...
            with torch.no_grad():
                ...
                topK_direction = torch.from_numpy(perturbation_summed).cuda(device=self.device)
